...
export function ImportPriceModal({
  open,
  onClose,
  onUpload,
  existing_companys = [],
    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div className="bg-white border border-gray-300 w-[750px] rounded-sm p-6 max-h-[90vh] overflow-y-auto shadow-xl">
        ...
        <div className="flex justify-between items-center mb-5">
          <h2 className="text-lg text-gray-900 font-medium">Import Price Data</h2>
          ...
          {existing_companys.length === 0 ? (
            ...
            <div className="max-h-48 overflow-y-auto border border-gray-200 rounded-sm bg-white">
              {filteredCompanies.length === 0 ? (
                <p className="p-3 text-sm text-gray-500">No matching companies found.</p>
              ) : (
                filteredCompanies.map((c) => (
                  ...
                  >
                    <span className="text-blue-600 font-medium">{c.ticker}</span>
        ...
        {/* CSV Upload */}
        <div className="mb-6">
          <label className="block text-sm text-gray-600 mb-2">Upload CSV File</label>
          <input
            type="file"
            accept=".csv"
            onChange={(e) => setFile(e.target.files?.[0] ?? null)}
            className="w-full text-sm text-gray-700 bg-white border border-gray-300 p-2 rounded-sm 
              file:mr-4 file:py-1 file:px-3 file:rounded-sm file:border-0 
